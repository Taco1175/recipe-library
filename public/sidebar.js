...
const ICONS = {
  logo: `<svg viewBox="0 0 16 16" fill="currentColor"><path d="M8 1a4 4 0 0 1 4 4c0 1.5-.8 2.8-2 3.5V14a1 1 0 0 1-2 0v-1H8v1a1 1 0 0 1-2 0V8.5C4.8 7.8 4 6.5 4 5a4 4 0 0 1 4-4z"/></svg>`,
  collapse: `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M10 12L6 8l4-4"/></svg>`,
  expand: `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M6 4l4 4-4 4"/></svg>`,
  menu: `<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M2 4h12M2 8h12M2 12h12"/></svg>`,
...
// Inject SVG icons into sidebar nav items by replacing text-based icons
function injectSidebarIcons() {
  // Logo mark
  const logoMark = document.querySelector('.sidebar-logo-mark');
  if (logoMark) logoMark.innerHTML = ICONS.logo;

  // Collapse button
  const colBtn = document.getElementById('sidebar-collapse-btn');
  if (colBtn) {
    const isCollapsed = document.getElementById('sidebar')?.classList.contains('collapsed');
    colBtn.innerHTML = isCollapsed ? ICONS.expand : ICONS.collapse;
  }
  ...
  // Nav icons by data-icon attribute
  ...
  // Sign out icon
  const signIcon = document.querySelector('.sidebar-signout .sign-icon');
  if (signIcon) signIcon.innerHTML = ICONS.signout;

  // Backfill button icon
  const backfillBtn = document.getElementById('backfill-btn');
  if (backfillBtn) {
    const txt = backfillBtn.textContent.replace(/^[^\w]+/, '').trim();
    backfillBtn.innerHTML = `${ICONS.image}<span>${txt}</span>`;
  }
}
...
function toggleSidebarCollapse() {
  ...
  if (btn) btn.innerHTML = isCollapsed ? ICONS.expand : ICONS.collapse;
}
...
// bfcache: reload on back/forward
